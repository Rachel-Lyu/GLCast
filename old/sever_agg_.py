...
class HealthDataHandler:
    ...
    def get_informative_states(self, primary_state, state_method, auxiliary_state_list = None, nStates = 5):
        ...
        if state_method == 'marginal_diff':
            ...
            if nStates is not None:
                ...
                print(self.informative_state_list)
    ...
    def Q_aggregation(self, B, X_test, y_test, total_step = 10):
        ...
        # theta.hat[khat] <- 1
        # beta=B[,khat]
        khat = np.argmin(np.mean((y_test - np.dot(X_test, B)) ** 2, axis = 0))
        ...
        # theta_hat = np.exp(-np.mean((y_test - np.dot(X_test, B)) ** 2, axis = 0) / 2)
    ...
    def run_all_agg(self, state_method, auxiliary_state_list = None, nStates = 5, total_step = 10):
        ...
        for primary_state in self.state_list:
            ...
            B = B.T
            theta_hat, beta = self.Q_aggregation(B, self.grouped_X_test[primary_state], self.y_test[primary_state], total_step)
            print(theta_hat)
            self.recovered_coef_agg[primary_state] = beta
            self.recovered_intercept_agg[primary_state] = recover_intercept(self.grouped_X_test[primary_state], self.y_test[primary_state], beta, intercept_length)
### grpLasso
# SIS - grpLasso - KKT
...
if __name__ == "__main__":
    ...
    state_list = ['VT', 'MA', 'ME', 'NH', 'CT', 'RI', 'PR', 'NJ', 'NY', 'PA', 'VA']
    ...
    data_handler.run_all_agg(state_method = 'marginal_diff', auxiliary_state_list = None, nStates = 5, total_step = 10)